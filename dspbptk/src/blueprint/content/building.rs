...
// TODO 重构+续写，注意零成本抽象
// 我希望重构并续写这段代码。这段代码中有一个结构体BuildingData，在实际应用中大量的BuildingData构成一个结构体数组，储存了所有建筑信息。但是建筑分成若干种，每一种建筑仅仅用到其中的一部分字段，并且有各不相同的parameters，这些parameters可以被反序列化为一个固定的参数列表。
// 所以我希望在BuildingData之上增加一层抽象，给每一种建筑定义一个结构体，然后根据item_id和model_index可以区分当前建筑的类型，从而存入对应的结构体内。我倾向于使用枚举实现，但是你也可以选择更合适的做法，最后所有的建筑仍然需要可以方便的保存在一个数组里。对于未知建筑，请保存为“未知建筑”，同时原样保留参数列表。
// 要注意这段代码性能敏感，请尽量减少不必要的性能开销
