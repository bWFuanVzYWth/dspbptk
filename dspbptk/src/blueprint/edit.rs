...
pub fn local_offset_to_direction(local_offset_x: f32, local_offset_y: f32) -> Vector3<f64> {
    ...
    let x_normal = r_normal * (local_offset_x as f64 * (PI / HALF_EQUATORIAL_GRID)).sin();
    let y_normal = r_normal * (local_offset_x as f64 * (PI / HALF_EQUATORIAL_GRID)).cos();
...
pub fn direction_to_local_offset(direction: &Vector3<f64>) -> (f32, f32) {
    ...
    let x = (direction.y / (1.0 - direction.z * direction.z).sqrt()).acos()
        * if direction.x >= 0.0 {
...
mod test {
    ...
    #[test]
    fn test_direction_to_local_offset_origin() {
    ...
    #[test]
    fn test_direction_to_local_offset_north_pole() {
